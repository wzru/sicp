...
async function translateXml(filepath, filename, option) {
    ...
    if (parseType == 'web') {
        ...
        if (option == 'generateTOC') {
            ...
            const outputFile = path.join(outputDir, '/chapters', tableOfContent[relativeFilePath].index + '.html');
            const stream = fs.createWriteStream(outputFile);
            stream.once('open', fd => {
                stream.write(writeTo.join(''));
                stream.end();
    ...
    if (parseType == 'js') {
        ...
        console.log(path.join(filepath, filename));
        const relativeFileDir = path.join(outputDir, filepath, filename.replace(/\.xml$/, '') + '');
        ensureDirectoryExists(path.join(outputDir, filepath), err => {
        });
